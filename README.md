...
Phase 2 compiles control flow, arrays/`GOTO`, and `SUB` recursion. Phase 3 adds interactive `INPUT` on a unified console. Phase 4 opens Amiga-style `SCREEN` / `WINDOW` chrome in the Display panel (try **WINDOW / SCREEN**). Phase 5 draws with `LINE` / `CIRCLE` / `PSET` (try **Graphics**). Phase 5+ adds `PAINT` / `AREA` / `PATTERN` (try **PAINT / PATTERN**). Phase 5.5 adds math builtins (`RND`, `SQR`, …) — try **Hi There!** or **Ahl benchmark**. Phase 6 plays tones — try **SOUND / WAVE** (unmute the tab). Phase 7 draws with the turtle — try **Turtle flower** or **Turtle torus**.
